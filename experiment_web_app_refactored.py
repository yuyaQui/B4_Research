...
def initialize_session_state():
    ...
    if 'experiment_set' not in st.session_state:
        try:
            ...
            st.error(f"データファイルが見つかりません: {os.path.join(DATASETS_PATH, 'sample_quizes_and_images.pkl')}")
...
def process_image_pattern2(quiz_data, index):
    ...
    try:
        ...
        # 下部にテキストを描画
        image_with_caption = find_lower_text_position_and_draw(image_copy, answer)
