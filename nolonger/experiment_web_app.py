...
def run_gaze_tracker(stop_event, result_container):
    ...
    # カメラ起動
    cap = cv2.VideoCapture(1) # 映らない場合は 1 に変更
    if not cap.isOpened():
...
with tab3:
    ...
    if "tracker_thread" not in st.session_state:
        ...
        st.session_state.result_container = {"distance": 0.0}
    ...
    if not st.session_state.processed_images_p1:
        ...
        if curr_idx < len(st.session_state.processed_images_p1):
            ...
            if not st.session_state.p1_study_time_logged:
                ...
                print(f"\n--- [タブ3] パターン1 視線移動距離: {final_distance:.2f} ---")
                # ここまで追加
    ...
    if not st.session_state.processed_images_p2:
        ...
        if curr_idx < len(st.session_state.processed_images_p2):
            ...
            if not st.session_state.p2_study_time_logged:
                ...
                print(f"\n--- [タブ4] パターン2 視線移動距離: {final_distance:.2f} ---")
                # ここまで追加
